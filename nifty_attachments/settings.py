...
ATTACHMENTS_CONTENT_TYPE_WHITELIST = getattr(
    settings, "ATTACHMENTS_CONTENT_TYPE_WHITELIST", None
) or getattr(settings, "FILE_UPLOAD_CONTENT_TYPES", ())

# max. size in Mb, None for no limit
ATTACHMENTS_FILE_UPLOAD_MAX_SIZE = getattr(
    settings, "ATTACHMENTS_FILE_UPLOAD_MAX_SIZE", None
) or getattr(settings, "FILE_UPLOAD_MAX_SIZE", 10)

# an iterable of uploaded file validators or a dotted path to import such an iterable.
ATTACHMENTS_FILE_UPLOAD_VALIDATORS = getattr(
    settings, "ATTACHMENTS_FILE_UPLOAD_VALIDATORS", None
) or getattr(
    settings,
