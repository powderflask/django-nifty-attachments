...
def upload_file(
    client, attachment_model, obj, file_obj=None, file_content=b"file content", **extra
):
...
def get_related_set(attachment_model, obj):
    ...
    return (
        obj.uuid_attachments.all()
        if attachment_model is UuidAttachment
        else obj.attachment_set.all()
    )
...
def test_empty_post_to_form_wont_create_attachment(
    attachment_model, related_object, logged_in_client_user
):
...
@pytest.mark.django_db
def test_invalid_attachment_wont_fail(
    attachment_model, related_object, logged_in_client_user
):
    client, user = logged_in_client_user
    response = upload_file(
        client, attachment_model, related_object, file_obj="Not a UploadedFile object"
    )
    # uuid model requires extra permissions to add, so yields Permission Denied
    assert response.status_code == 403 if attachment_model is UuidAttachment else 200
    assert attachment_model.objects.count() == 0
...
@pytest.mark.parametrize("attachment_factory", [GizmoAttachmentFactory], indirect=True)
@pytest.mark.django_db
def test_upload_size_less_than_limit(
    attachment_settings, attachment_model, related_object, logged_in_client_user
):
...
@pytest.mark.parametrize("attachment_factory", [GizmoAttachmentFactory], indirect=True)
@pytest.mark.django_db
def test_upload_size_more_than_limit(
    attachment_settings, attachment_model, related_object, logged_in_client_user
):
...
@pytest.mark.parametrize("attachment_factory", [GizmoAttachmentFactory], indirect=True)
@pytest.mark.django_db
def test_upload_with_permission(
    attachment_model, related_object, logged_in_client_user
):
    client, user = logged_in_client_user
    response = upload_file(client, attachment_model, related_object)
...
@pytest.mark.django_db
def test_upload_with_custom_permission(
    attachment_model, related_object, logged_in_client_user
):
    client, user = logged_in_client_user
    response = upload_file(client, attachment_model, related_object)
    assert user.has_perm(
        get_perm_name_for_model(attachment_model, "add")
    )  # has basic perm
...
@pytest.mark.django_db
def test_owner_cant_delete_attachment_without_permission(
    client, attachment_model, attachment
):
    user = attachment.owner
...
@pytest.mark.django_db
def test_cant_delete_others_attachment_without_permission(
    attachment_model, attachment, logged_in_client_user
):
    client, user = logged_in_client_user
...
@pytest.mark.django_db
def test_can_delete_others_attachment_with_permission(
    client, attachment_model, attachment, get_user_factory
):
...
@pytest.mark.parametrize("attachment_factory", [GizmoAttachmentFactory], indirect=True)
@pytest.mark.django_db
def test_custom_validator_denies_specific_content(
    attachment_model, related_object, logged_in_client_user
):
    client, user = logged_in_client_user
...
@pytest.mark.django_db
def test_non_owner_cant_update_attachment_without_edit_any(
    attachment_model, attachment, logged_in_client_user
):
...
@pytest.mark.django_db
def test_non_owner_can_update_with_edit_any_permission(
    client, attachment_model, attachment, get_user_factory
):
...
@pytest.mark.django_db
def test_list_attachments_finds_correct_relation(
    client, attachment_model, attachment, get_user_factory
):
    """
    ...
    # URL for list view
    url = reverse(
        f"{attachment_model.get_url_namespace()}:list",
        args=(attachment.related_object_id,),
    )
...
@pytest.mark.django_db
def test_redirect_to_next_parameter(
    client, attachment_model, related_object, logged_in_client_user
):
    ...
    file_obj = SimpleUploadedFile(
        "test.pdf", b"fake binary content", content_type="application/pdf"
    )
...
class TestViewResolutionFailures:
    ...
    def test_update_view_invalid_attachment_id(
        self, client, attachment, attachment_model, logged_in_client_user
    ):
        client, _ = logged_in_client_user
        ...
        url = reverse(
            f"{attachment_model.get_url_namespace()}:update",
            args=(attachment.related_object_id, fake_pk),
        )

        response = client.post(url, data={"label": "New"})
        assert response.status_code == 404

    def test_delete_view_invalid_attachment_id(
        self, client, attachment, attachment_model, logged_in_client_user
    ):
        client, _ = logged_in_client_user
        ...
        url = reverse(
            f"{attachment_model.get_url_namespace()}:delete",
            args=(attachment.related_object_id, fake_pk),
        )
...
@pytest.mark.django_db
def test_mixin_invalid_request_handling(
    client, attachment_model, logged_in_client_user
):
    client, _ = logged_in_client_user
...
def test_list_permission_denied(client, attachment, logged_in_client_user):
    ...
    client.force_login(stranger)
    url = reverse(
        f"{attachment.get_url_namespace()}:list", args=(attachment.related_object.pk,)
    )
...
@pytest.mark.django_db
def test_update_view_invalid_request_mismatch(
    client, attachment, attachment_model, admin_user
):
    """
    ...
    url = reverse(
        f"{attachment_model.get_url_namespace()}:update",
        args=(other_obj.pk, attachment.pk),
    )
...
@pytest.mark.django_db
def test_delete_view_invalid_request_mismatch(
    client, attachment, attachment_model, admin_user
):
